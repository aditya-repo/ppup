...
const getCaptchaText = require('./anti-captcha');
const increaseImageSize = require('./image-process');
...
async function submitForm(browser, form) {
    ...
    await page.goto('https://ppuponline.in/Student/RegistrationSearch.php'); // Replace with your form URL
  ...
  const captchaPath = path.join(__dirname, 'captcha', `${form.id}.png`)
  ...
  const captchaPath2 = path.join(__dirname, 'captcha', `${form.id}.png`)

  await fs.writeFile(captchaPath2, captchaImageScreenshot2, 'binary');

  await increaseImageSize(captchaPath2)
